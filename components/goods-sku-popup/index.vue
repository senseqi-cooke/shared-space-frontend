<template>
  ...
  >
    ...
    <view
      ...
      :style="
        'border-radius: ' + borderRadius + 'rpx ' + borderRadius + 'rpx 0 0;'
      "
    >
          ...
          <view class="specification-header">
            ...
            <view class="specification-right">
              ...
              >
                ...
                <text class="price">
                  {{
                    computedPrice(selectShop.price || defaultPrice)
                      | priceFilter
                  }}
                </text>
              </view>
              <view class="inventory">
                {{ stockText }}：{{ selectShop[stockName] || defaultStock }}
              </view>
              <view
                class="choose"
                v-show="
                ...
                "
              >
                <text v-if="!selectArr.every((val) => val == '')">
                  已选：{{ selectArr.join(" ") }}
                </text>
              </view>

              <view v-show="serviceIndex.length" class="service">
                服务：{{ selectService }}
              </view>
            </view>
          </view>
          <scroll-view class="specification-content" scroll-y="true">
            <view
              v-show="goodsInfo[specListName][0].name !== defaultSingleSkuName"
            ...
            >
              ...
              <view
                class="item-wrapper"
                :style="{ display: isAI == 1 ? 'block' : 'flex' }"
              >
                <view
                ...
                >
                  <view
                    ...
                    :style="[
                      item_value.ishow ? '' : disableStyle,
                      item_value.ishow ? btnStyle : '',
                      subIndex[index1] == index2 ? activedStyle : '',
                    ]"
                  >
                    <view>{{ item_value.name }}</view>
                    <view v-if="item_value.price != -1">
                      ￥{{ item_value.price | priceFilter }}
                    </view>
                  </view>
                </view>
              </view>
            ...
            <view
              class="specification-item"
              v-for="(item, index1) in goodsInfo.serviceList"
              :key="index1"
            >
              <view class="item-title">{{ item.name }}</view>
              <view class="item-wrapper" @tap="serviceClick(item)">
                <view
                  class="item-content"
                  :class="[serviceIndex.includes(item.id) ? 'actived' : '']"
                  :style="[
                    btnStyle,
                    serviceIndex.includes(item.id) ? activedStyle : '',
                  ]"
                >
            ...
            <view style="display: flex">
              <view style="flex: 1">
                <text style="font-size: 26rpx; color: #333; line-height: 50rpx">
                  数量
                </text>
              </view>
      ...
      <view class="btn-option">
        ...
        <view class="btn-wrapper" v-else-if="mode == 1">
          <view
            ...
            @click="addCart"
            :style="
              'color:' + addCartColor + ';background:' + addCartBackgroundColor
            "
            >{{ addCartText }}</view
          >
          <view
        ...
        <view class="btn-wrapper" v-else-if="mode == 3">
          <view
            ...
            @click="buyNow"
            :style="
              'color:' + buyNowColor + ';background:' + buyNowBackgroundColor
            "
            >{{ buyNowText }}</view
          >
        </view>
...
export default {
  ...
  props: {
    ...
    closeImage: {
      Type: String,
      default:
        "https://img.alicdn.com/imgextra/i1/121022687/O1CN01ImN0O11VigqwzpLiK_!!121022687.png",
    },
    // 默认库存数量 (未选择sku时)
    defaultStock: {
      Type: Number,
  ...
  methods: {
    ...
    init() {
      ...
      //默认选中第一个
      // const firstSKU = that.goodsInfo[that.specListName][0];
      // console.log("first=",firstSKU);
      // if (firstSKU){
      //   that.skuClick(firstSKU.list[0], 0,null,0);
      // }
      //默认选中必选
      that.goodsInfo[that.specListName].map((item, index) => {
        const itemInfo = item.list[0];
        if (index == 0) {
          //默认选中第一个规格的第一个sku
          that.skuClick(itemInfo, index,null,0);
        }else {
          //只有一个sku的也默认选中
    ...
    async open() {
      ...
      if (that.customAction && typeof that.customAction === "function") {
        ...
        if (
          ...
          that.toast({ title: "无法获取到商品信息", icon: "none" });
    ...
    skuClick(value, index1, event, index2) {
      // console.log("skuClick", value, index1, index2)
      if (value.ishow) {
        // 当前选项不在已选数组中时，将其添加到已选数组和子索引对象中
        if (that.selectArr[index1] != value.name) {
          that.$set(that.selectArr, index1, value.name);
          that.$set(that.subIndex, index1, index2);
        } else {
          // 当前选项已在已选数组中，从已选数组和子索引对象中移除
          //只有一个sku的不能移除
          const thisSkuList = that.goodsInfo[that.specListName][index1].list;
          // console.log("thisSkuList=",thisSkuList)
          if (thisSkuList.length == 1){
            that.toast("该规格不能取消", "none");
            return false;
          }
    ...
    // 服务点击
    serviceClick(value) {
      if (that.serviceIndex.includes(value.id)) {
        that.serviceIndex = that.serviceIndex.filter((el) => el != value.id);
      } else {
        that.serviceIndex.push(value.id);
      }
    },
    ...
    computedPrice(price) {
      ...
      if (this.serviceIndex.length) {
        const serviceList = this.goodsInfo.serviceList;
        const servicePrice = (serviceList || [])
          .filter((el) => {
            return (this.serviceIndex || []).includes(el.id);
          })
          .reduce((sum, cur) => {
            const { price } = cur;
        ...
        return addDecimals(price, servicePrice, 2);
      }
      return price;
    },
    ...
    addCart() {
      that.checkSelectComplete({
        success: function (selectShop) {
          ...
          selectShop.additionalServiceIds = (that.serviceIndex || []).join(",");
    ...
    autoClickSku() {
      ...
      if (specListArr.length == 1) {
        ...
        for (let i = 0; i < specList.length; i++) {
          let sku = that.getListItem(skuList, that.skuArrName, [
            specList[i].name,
          ]);
          if (sku) {
  ...
  computed: {
    selectService() {
      ...
      return (serviceList || [])
        .filter((el) => {
          return (that.serviceIndex || []).includes(el.id);
        })
        .map((el) => el.name || "-")
        .join(", ");
    },
