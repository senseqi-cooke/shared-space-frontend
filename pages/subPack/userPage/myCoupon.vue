		...
		<view class="couponList"> <!-- @click="goPage('/pages/subPack/discounts/couponDetail')" -->
			<view class="couponLi" 
				...
				<view class="center">
					...
					<view class="condition" v-if="item.type === 'N'">
						使用条件：每次可用{{item.everyUseableHourCount}}小时,剩余可用{{item.remUseableCount}}次数
					</view>
					<view class="condition" v-if="item.type === 'H'">
						使用条件：剩余可用{{item.remHourCount}}小时数
					</view>
					<view class="isMsg">
						<!-- <text>查看详情></text> -->
