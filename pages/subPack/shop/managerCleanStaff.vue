...
<!-- 		<view class="topBox">
		...
		<view class="teamList" v-if="staffList.length > 0">
			
			<view class="teamBox" v-for="(item,index) in staffList" :key="index">
				...
				<view @click="goPageEdit('/pages/subPack/shop/editCleanStaff',item)">
					<text>编辑</text>
				</view>
...
<script>
	...
	import { getStaffList } from '@/api/mtstore';	
	...
	export default {
		...
		methods: {
			...
			getStaffList() { // 获取员工列表
				...
				getStaffList(paramData).then(res => {
					if (res.status == 200) {
