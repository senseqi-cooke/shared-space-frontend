<template>
  <view v-if="!isLoading" class="container">
      ...
      <view class="order-status">
        <view class="status-icon">
          ...
          <block>
            ...
            <block>
              <image
                v-if="order.status == OrderStatusEnum.CREATED.value"
                class="image"
                src="/static/order/status/wait_pay.png"
                mode="aspectFit"
              ></image>
            </block>
            <!-- 已支付 -->
            <block>
              <image
                v-if="order.status == OrderStatusEnum.PAID.value"
                class="image"
                src="/static/order/status/received.png"
                mode="aspectFit"
              ></image>
            </block>
            <!-- 待发货 -->
            <block>
              <image
                v-if="order.status == OrderStatusEnum.DELIVERY.value"
                class="image"
                src="/static/order/status/wait_deliver.png"
                mode="aspectFit"
              ></image>
            </block>
            <!-- 已发货 -->
            <block>
              <image
                v-if="order.status == OrderStatusEnum.DELIVERED.value"
                class="image"
                src="/static/order/status/wait_receipt.png"
                mode="aspectFit"
              ></image>
            </block>
            <!-- 已收货-->
            <block>
              <image
                v-if="order.status == OrderStatusEnum.RECEIVED.value"
                class="image"
                src="/static/order/status/received.png"
                mode="aspectFit"
              ></image>
            ...
            <block>
              <image
                v-if="
                  order.status == OrderStatusEnum.CANCEL.value ||
                  order.status == OrderStatusEnum.REFUND.value
                "
                class="image"
                src="/static/order/status/close.png"
                mode="aspectFit"
              ></image>
            </block>
          </block>
        ...
        <view class="status-text">
          <text v-if="order.status == OrderStatusEnum.CREATED.value">{{
            OrderStatusEnum.CREATED.name
          }}</text>
          <text v-else-if="order.status == OrderStatusEnum.PAID.value">{{
            OrderStatusEnum.PAID.name
          }}</text>
          <text v-else-if="order.status == OrderStatusEnum.DELIVERY.value">{{
            OrderStatusEnum.DELIVERY.name
          }}</text>
          <text v-else-if="order.status == OrderStatusEnum.DELIVERED.value">{{
            OrderStatusEnum.DELIVERED.name
          }}</text>
          <text v-else-if="order.status == OrderStatusEnum.RECEIVED.value">{{
            OrderStatusEnum.RECEIVED.name
          }}</text>
          <text v-else-if="order.status == OrderStatusEnum.CANCEL.value">{{
            OrderStatusEnum.CANCEL.name
          }}</text>
          <text v-else-if="order.status == OrderStatusEnum.REFUND.value">{{
            OrderStatusEnum.REFUND.name
          }}</text>
        </view>
        <view
          class="verify-code"
          v-if="
            order.orderMode == 'oneself' &&
            order.verifyCode &&
            order.payStatus == 'B'
          "
        >
          <view v-if="storeRoomId <= 0">核销码：</view>
          <view class="code" v-if="storeRoomId <= 0">{{
            order.verifyCode
          }}</view>
        </view>
      </view>
    ...
    <view v-if="order.orderMode == 'oneself'" class="delivery-address i-card">
      <view class="link-man" v-if="storeRoomId <= 0">
    ...
    <view class="goods-list i-card" v-if="order.goods.length > 0">
      <view class="goods-item" v-for="(goods, idx) in order.goods" :key="idx">
        <view
          class="goods-main"
          v-if="goods.num > 0"
          @click="handleTargetGoods(goods.goodsId, goods.type)"
        >
          <!-- 商品图片 -->
          ...
          <view class="goods-content">
            ...
            <view
              ...
              v-show="goods.additionalList && goods.additionalList.length"
            >
              已选服务:
              <text v-for="(props, idx) in goods.additionalList" :key="idx">
                {{ props.name }};
              </text>
            </view>
          </view>
    ...
    <view class="trade-info i-card">
      ...
      <view class="trade-total">
        ...
        <view class="goods-price">
          <text class="unit">￥</text>
          <text class="value">{{
            order.payAmount ? order.payAmount.toFixed(2) : "0.00"
          }}</text>
        </view>
      </view>
    </view>
    ...
    <view
      class="footer-fixed"
      v-if="order.status == OrderStatusEnum.PAID.value && storeRoomId <= 0"
    >
      <view class="btn-wrapper">
        <block v-if="!order.refundInfo">
          <view class="btn-item active" @click="handleApplyRefund(order.id)"
            >申请售后</view
          >
        </block>
        <block v-if="order.refundInfo">
          <view
            class="btn-item common"
            @click="handleRefundDetail(order.refundInfo.id)"
            >售后详情</view
          >
        </block>
      </view>
    ...
    >
      <view class="btn-wrapper">
        <block>
          <view class="btn-item active" @click="onReceipt(order.id)"
            >确认收货</view
          >
        </block>
      </view>
      ...
      <view class="pay-popup">
        ...
        <view class="pop-content">
          ...
          <view
            ...
            @click="onSelectPayType(PayTypeEnum.WECHAT.value)"
          >
		  ...
		  @click="onSelectPayType(PayTypeEnum.BALANCE.value)" style="display: none;">
            <view class="item-left dis-flex flex-y-center">
...
export default {
  ...
  methods: {
    ...
    handleApplyRefund(orderId) {
      this.$navTo("pages/shopmall/refund/apply", { orderId });
    },

    // 售后详情
    handleRefundDetail(refundId) {
      this.$navTo("pages/shopmall/refund/detail", { refundId });
    },
    ...
    onCancel() {
      ...
      uni.showModal({
        ...
        success(o) {
          if (o.confirm) {
            OrderApi.cancel(app.orderId).then((result) => {
              // 显示成功信息
