...
export function formatDateTimeByTimeZone(dateTimeString) {
  // 假设输入的dateTimeString格式总是 "yyyy-MM-ddTHH:mm:ss.SSS+ZZ:ZZ"
  const [datePart, timePart, _] = dateTimeString.split("T");
  const [hours, minutes] = timePart.split(":").slice(0, 2); // 只取小时和分钟
...
export function dateFormatT(time) {
  ...
  return (
    d.getFullYear() + "/" + (d.getMonth() + parseInt(1)) + "/" + d.getDate()
  );
}
        ...
        auth(code)
          .then(() => {
            ...
            if (redirect) {
              ...
              if (!redirect) {
                redirect = "/homePage_V2/homePage_V2";
              }
              reLaunch({
                path: "/pages" + redirect,
              });
...
export const handleLoginStatus = (location, complete, fail, success) => {
  ...
  let page = [
    ...
    {
      path: "/pages/subPack/homePage/details",
      name: "房间详情页",
    },
    {
      path: "/pages/subPack/homePage/selectCity",
      name: "选择城市页",
    },
    /*    {
  ...
  console.log("parseUrl_location:" + parseUrl(location));
  return new Promise((resolve, reject) => {
    resolve({
...
export const handleLoginFailure = () => {
  ...
  if (store.state.$deviceType == "weixin") {
    ...
    if (
      store.getters.isAuthorizationPage ||
      currentPageUrl == "/pages/user/Login/index"
    ) {
      console.log(
        store.getters.isAuthorizationPage,
        currentPageUrl,
 ...
 */
// 日期格式化
export function parseTime1(time, pattern) {
...
function trimTrailingZeros(str) {
  return str.replace(/(\.\d*[1-9])(\d*)$/, '$1').replace(/\.$/, '');
}

export function addDecimals(num1, num2, precision = 3) {
  // 将数字转换为字符串，然后分割成整数部分和小数部分
  const [, decimalPart1] = num1.toString().split(".");
  const [, decimalPart2] = num2.toString().split(".");

  // 确定小数位数最多的那个数
  const maxDecimalLength = Math.max(
    decimalPart1 ? decimalPart1.length : 0,
    decimalPart2 ? decimalPart2.length : 0
  );

  // 如果两个数都没有小数部分，则不需要转换
  if (maxDecimalLength === 0) {
    return (num1 + num2).toFixed(precision); // 假设你想要结果保留两位小数
  }

  // 计算倍数
  const multiplier = 10 ** maxDecimalLength;

  // 转换小数到整数并相加
  const intSum = Math.round(num1 * multiplier) + Math.round(num2 * multiplier);

  return trimTrailingZeros((intSum / multiplier).toFixed(precision));
}

export function subtractDecimals(num1, num2, fixedDigits = 3) {
  // 将数字转换为字符串，然后分割成整数部分和小数部分
  const [, decimalPart1] = num1.toString().split(".");
  const [, decimalPart2] = num2.toString().split(".");
  ...
  const intNum2 = Math.round(num2 * multiplier);

  // 执行减法
  const intDiff = intNum1 - intNum2;

  // 将结果转换回小数并保留指定的小数位数
  return (intDiff / multiplier).toFixed(fixedDigits);
}
